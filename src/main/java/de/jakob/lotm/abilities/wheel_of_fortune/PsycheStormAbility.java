...
public class PsycheStormAbility extends Ability {
    ...
    public void onAbilityUse(Level level, LivingEntity entity) {
        ...
        AbilityUtil.getNearbyEntities(entity, serverLevel, entity.getEyePosition(), 10*multiplier).forEach(e ->
                e.addEffect(new MobEffectInstance(ModEffects.LOOSING_CONTROL, 20 * 7, getAmplifier(entity, e))));
        int seq = BeyonderData.getSequence(entity);
        if(seq <= 4){
            AbilityUtil.getNearbyEntities(entity, serverLevel, entity.getEyePosition(), 10).forEach(e -> e.getData(ModAttachments.SANITY_COMPONENT).increaseSanityAndSync((float) (-0.25f * multiplier(entity)), e));
