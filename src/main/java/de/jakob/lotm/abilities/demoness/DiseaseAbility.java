...
public class DiseaseAbility extends Ability {
    ...
    public void onAbilityUse(Level level, LivingEntity entity) {
        ...
        ServerScheduler.scheduleForDuration(0, 20, 20 * 40*(int) Math.max(multiplier(entity)/4,1), () -> {
            ...
            AbilityUtil.addPotionEffectToNearbyEntities((ServerLevel) entity.level(), entity, 40*(int) Math.max(multiplier(entity)/4,1), entity.position(), new MobEffectInstance(MobEffects.POISON, 20, 0, false, false, false));
            AbilityUtil.damageNearbyEntities((ServerLevel) entity.level(), entity, 40*(int) Math.max(multiplier(entity)/4,1), (float) DamageLookup.lookupDps(5, .2, 20, 20) *(int) Math.max(multiplier(entity)/6,1) * damageMult, entity.position(), true, false, true, 0, ModDamageTypes.source(level, ModDamageTypes.DEMONESS_GENERIC, entity));
